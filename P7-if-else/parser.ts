...
export class Parser {
    ...
    private eatToken(token: TokenType) {
        if (this.currentToken.type === token) {
            this.advance();
        } else {
            throw new Error(`Invalid Syntax: Token of type ${token} is required.`);
        }
    }
    ...
    private block(): Node {
        ...
        if (this.currentToken.type === TokenType.L_C_BRACKET) {
            this.advance();
            isScoped = true;
        }

        while (this.currentToken && this.currentToken.type !== TokenType.TT_EOF) {
            if (this.currentToken.type === TokenType.R_C_BRACKET) {
                if (isScoped) {
                    this.advance();
                    break;
                } else {
                    throw new Error(`Invalid syntax: Closing scope "}" without any openig one.\n${block}`);
                }
            }
    ...
    private statement(): Node {
        let state: Node;
        if (this.currentToken.type === TokenType.TT_KEYWORD && this.currentToken.keyword === KeywordType.KW_VAR) {
        ...
        else if (this.currentToken.type === TokenType.TT_KEYWORD && this.currentToken.keyword === KeywordType.KW_IF) {
            this.currentToken = null;
            this.advance();
            this.eatToken(TokenType.TT_L_PARAN);
            const cond = this.assignment();
            this.eatToken(TokenType.TT_R_PARAN);
            const block = this.block();
            state = { type: NodeType.NT_STATE, statementType: 'if', condition: cond, ifBlock: block }
            if (this.currentToken.type === TokenType.TT_KEYWORD && this.currentToken.keyword === KeywordType.KW_ELSE) {
                this.advance();
                const else_block = this.block();
                state.elseBlock = else_block
            }
