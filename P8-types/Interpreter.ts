...
export class Interpreter {
    ...
    private visit_statement(node: Node, context: Context) {
        switch (node.statementType) {
            ...
            case 'declear': {
                ...
                context.addVar(node.identifier, val)
                return val;
            }
            case 'if': {
                const cond_res = this.visit(node.condition, context)
                if (cond_res) {
                    return this.visit(node.ifBlock, context)
                } else if (node.elseBlock) {
                    return this.visit(node.elseBlock, context)
                }
            }
        }
    }
    ...
    private visit_assignment(node: Node, context: Context): any {
        ...
        switch (node.assignmentType) {
            case 'assign': {
                const val = this.visit(node.nodes as Node, context);
                context.setVar(node.identifier, val)
                return val;
            }
            case 'logical':
                return this.visit(node.nodes as Node, context)
